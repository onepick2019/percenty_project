...
"""
퍼센티 Step 3 배치 실행기
새로운 Step3 Core 파일들을 사용하여 서버별 배치 처리 수행
"""
...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler('step3_batch_runner.log', encoding='utf-8'),
        logging.StreamHandler()
    ]
)
logger = logging.getLogger(__name__)

class Step3BatchRunner:
    """
    Step 3 배치 실행기
    서버별로 독립적인 배치 처리 수행
    """
                ...
                try:
                    ...
                    while remaining_keywords:
                        if server == "서버1":
                            ...
                            current_result = step_core.execute_step3_2_with_browser_restart(
                                provider_codes=remaining_keywords,
                                chunk_size=chunk_size,
                                account_info=account_info
                            )
    ...
    def _reset_progress_files(self, account_info: Dict, server_name: str = None):
        ...
        try:
            ...
            for server in servers_to_reset:
                progress_file = f"progress_{account_id}_step3_{server}.json"
    ...
    def cleanup(self, force_process_cleanup=True):
        ...
        리소스 정리
        Args:
            force_process_cleanup (bool): Chrome/ChromeDriver 프로세스 강제 종료 여부
                                        GUI 모드에서는 False로 설정하여 다른 터미널 보호
        """
        try:
            ...
            # GUI 모드에서는 프로세스 강제 종료를 건너뛰어 다른 터미널 보호
...
def main():
    ...
    # 명령행 인자 파싱
    parser = argparse.ArgumentParser(description='퍼센티 Step 3 배치 실행기')
