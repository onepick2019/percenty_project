...
class Step6_2Core:
    ...
    def __init__(self, driver=None):
        ...
        logger.info("Step6_2Core 초기화 완료")
    ...
    def execute_step(self, account_id: str, quantity: int = 100, **kwargs) -> Dict[str, Union[bool, str, int]]:
        ...
        try:
            ...
            logger.info(f"6-2단계 시작 - 계정: {account_id}, 수량: {quantity}")
            ...
            logger.info(f"6-2단계 완료 - 계정: {account_id}, 처리된 수량: {processed_count}")
            
        except Exception as e:
            logger.error(f"6-2단계 실행 중 오류: {e}")
    ...
    def _execute_core_logic(self, account_id: str, quantity: int) -> int:
        ...
        """
        try:
            # ProductEditorCore6_Dynamic2 인스턴스 생성
            core_processor = ProductEditorCore6_Dynamic2(self.driver, account_id)
            ...
            logger.error(f"6-2단계 핵심 로직 실행 중 오류: {e}")
            return 0
...
# 하위 호환성을 위한 함수들
def execute_step6_2(account_id: str, quantity: int = 100, headless: bool = False, driver=None, **kwargs) -> Dict[str, Union[bool, str, int]]:
    """
    6-2단계 실행 함수 (하위 호환성)
    ...
    if driver:
        ...
        try:
            ...
            # 6-2단계 실행 (브라우저가 이미 초기화되었으므로 headless 옵션 전달)
            result = step_core.execute_step(account_id, quantity, headless=headless, **kwargs)
...
def get_step_info() -> Dict[str, Union[str, int]]:
    ...
    return {
        'step_number': '6-2',
        'step_name': '6-2단계 등록상품관리 동적 업로드',
        'description': '등록상품관리에서 동적 그룹 선택 및 쿠팡 미업로드상품 등록',
...
def main():
    ...
    parser = argparse.ArgumentParser(description='6-2단계 등록상품관리 동적 업로드 실행')
